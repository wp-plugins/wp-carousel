<?php
	...
	*/
	...
	define('WP_CAROUSEL_VERSION', 1.00); // 0.6 = 0.60 < 0.6X < 0.70 == 0.7
	...
	*/
	...
	$wp_carousel_default_supported_supportable_features = array('stepcarousel', 'arrows', 'panel_size', 'image_size', 'pagination');
	$wp_carousel_default_unsupported_supportable_features = array('jcarousel', 'carousel_size', 'vertical_mode');
	...
	*/
	...
	/*
		Definimos la URL del repositorio de themes
	*/
	
	define('WP_CAROUSEL_THEME_SITE_URL', 'http://sumolari.com/wp-carousel-addons/');
	...
	*/
	...
	define('WP_CAROUSEL_ITEMS_COUNT_LIMIT', 1000); // 1000? 10000? I need more stats to set a better limit!
	...
	*/
	...
	wp_register_script('wp_carousel_jcarousel', $wp_carousel_path[6].'js/jcarousel.min.js', array('jquery'), false, true); // jCarusel, el script JS principal del carrusel (tomado de http://sorgalla.com/jcarousel/)
	wp_register_script('wp_carousel_stepcarousel', $wp_carousel_path[6].'js/stepcarousel.js', array('jquery'), false, true); // Stepcarousel, el antiguo script JS principal del carrusel (tomado de http://www.dynamicdrive.com/dynamicindex4/stepcarousel.htm)
	...
	function wp_carousel_load_extras($debug=false, $dir = '')
	{
		...
		if (isset($extras[0]))
		{
			...
			foreach ($extras as $temp_key => $temp_value)
			{
				...
				if (is_file($to_check_information_file))
				{
					...
					$log[] = 'Comienza el analisis de los archivos Javascript a cargar';
		...
		if ($debug)
		{
			...
			if(!empty($errors))
			{
				...
				$log[] = 'La comprobacion del recuento de errores de la funcion wp_carousel_load_extras() ha detectado que hay errores en la funcion wp_carousel_load_extras()';
	...
	*/
	...
	{	
		...
		$log[] = 'Comprobacion del valor de $debug en wp_carousel_get_theme_information()';
		if (!$debug)
		{
			$log[] = 'La comprobacion ha determinado que $debug no tiene por valor true (booleano)';
			// De momento sabemos que no estamos en modo debug, pero ¿será porque la variable es false o porque la variable no es true?
		...
		$log[] = 'Los themes soportan por defecto: '.print_r($wp_carousel_default_supported_supportable_features, true);
		$log[] = 'Los themes NO soportan por defecto: '.print_r($wp_carousel_default_unsupported_supportable_features, true);
		...
		foreach ($themes as $temp_key => $temp_value)
		{
			...
			if (is_file($to_check_information_file))
			{
				...
				{
					$log[] = 'Se ha localizado un archivo de theme (theme.php) en la carpeta';
					include($to_check_information_file);
				...
				{
					$log[] = 'No hay ningun archivo de theme (theme.php) en la carpeta: "'.$to_check_information_file.'"';
					$errors[] = 'No hay ningun archivo de theme (theme.php) en la carpeta: "'.$to_check_information_file.'"';
					$log[] = 'Se ha añadido a la matriz de errores el error '.key($errors).': "'.current($errors).'"';
				}
			}
			else
		...
		if ($debug)
		{
			$log[] = 'Comprobacion de recuento de errores de la funcion wp_carousel_get_theme_information()';
			if(!empty($errors))
			{
				// Uy uy uy... ha habido errores durante la ejecución de esta función, cortemos el script y mostremos los errores de forma legible
				$log[] = 'La comprobacion del recuento de errores de la funcion wp_carousel_get_theme_information() ha detectado que hay errores en la funcion wp_carousel_get_theme_information()';
				echo '<h2>'.__('Errors', 'wp_carousel').'</h2><pre>';
				print_r($errors);
				echo '</pre>';
				$log[] = 'Se ha mostrado el listado de errores de la funcion wp_carousel_get_theme_information()';
				...
				// ¡Qué bien, no hay errores!
				$log[] = 'La comprobacion del recuento de errores de la funcion wp_carousel_get_theme_information() ha determinado que no ha habido errores durante la ejecucion de wp_carousel_get_theme_information()';
				echo '<h2>'.__('Log', 'wp_carousel').'</h2><pre>';
				print_r($log);
				echo '</pre>';
			}
	...
	*/
	...
	{
		global $wp_carousel_thumbnail_size_line, $wp_carousel_external_integration_line, $wp_carousel_videos_first_line, $wp_carousel_autosave_backups_line;
		
		printf ("<p>".__('Please, fill up <a href="%s">this survey</a> in order to improve WP Carousel - Note that you can fill up the survey at any moment', 'wp_carousel').".</p>", WP_CAROUSEL_SURVEY);
		echo '<hr class="wp_carousel_help_separator" />';
		echo "<h5>".__('WP Carousel\'s Quick Help', 'wp_carousel')."</h5>";
		echo '<p>';
		printf(__('Did you find any error? Please, report them <a href="%s">here (English)</a> or <a href="%s">here (Spanish)</a>.', 'wp_carousel'), 'http://sumolari.com/forums/forum/wp-carousel-2/', 'http://sumolari.com/forums/forum/wp-carousel-2/');
		echo '</p>';
		echo '<hr class="wp_carousel_help_separator" />';
		
		
		echo "<h5>".__('My carousel does not work: it shows scrollbars and does not slide!', 'wp_carousel')."</h5>";
		echo '<p>';
		printf(__('Open your WordPress theme footer.php file and check if there\'s a line like: <code>%s</code>. If there is not any line like that, just add that code at the end of the file.', 'wp_carousel'), '&lt;?php wp_footer(); ?&gt;');
		echo '</p>';
		echo "<h5>".__('I have added a post with a video to the carousel and the video is not shown', 'wp_carousel')."</h5>";
		echo '<p>';
		_e('Some themes do not support videos. If your current WP Carousel theme supports videos, check if the size of the video is bigger than the size of the panel. If it is, the video won\'t be shown.', 'wp_carousel');
		echo '</p>';
		echo '<hr class="wp_carousel_help_separator" />';
		
		echo "<h5>".__('How to add items to a carousel', 'wp_carousel')."</h5>";
		echo '<p>';
		_e('Simply drag the kind of item you want to add and drop it in the carousel\'s area. When you have done that, you\'ll see new options to set up the item you want to add. ', 'wp_carousel');
		echo '</p>';
		
		echo "<h5>".__('How to change the carousel\'s name', 'wp_carousel')."</h5>";
		echo '<p>';
		_e('Click on the current name, change it and then press Enter. ', 'wp_carousel');
		echo '</p>';
		
		echo "<h5>".__('How to show a carousel', 'wp_carousel')."</h5>";
		echo '<p>';
		_e('You can show a carousel by three ways:', 'wp_carousel');
		printf('<ol><li>%s</li><li>%s</li><li>%s</li></ol>', sprintf(__('Adding <code>%s</code> in your WordPress theme.', 'wp_carousel'), 'wp_carousel(CAROUSEL_ID)'), __('Adding WP Carousel\'s Widget to the sidebar.', 'wp_carousel'), sprintf(__('Adding the shortcode <code>%s</code> in the post / page you want to show the carousel.', 'wp_carousel'), '[wp_carousel]ID[/wp_carousel]'));
		echo '</p>';
		echo "<h5>".__('WP Carousel\'s External Integration Mode', 'wp_carousel')."</h5>";
		echo '<p>';
		printf(__('With WP Carousel\'s External Integration Mode you can show a WP Carousel carousel from a WordPress blog in your WordPress blog. To do this, both blogs must have enabled this mode, which is disabled by default for security reasons. To enable it, just go to line %s in file %s, search %s and replace it with %s ', 'wp_carousel'), $wp_carousel_external_integration_line, '<code>'.__FILE__.'</code>', "<code>define('WP_CAROUSEL_EI', false);</code>", "<code>define('WP_CAROUSEL_EI', true);</code>");
		echo '</p>';
		echo '<hr class="wp_carousel_help_separator" />';
		
		echo "<h5>".__('Posts\'s images for the carousel', 'wp_carousel')."</h5>";
		printf("%s %s %s %s %s %s %s %s %s", "<p>", __('WP Carousel loads the image for the carousel in the following order:', 'wp_carousel'), "<ol><li>", __('Value of custom field', 'wp_carousel'), " <em>".WP_CAROUSEL_IMAGE_URL."</em></li><li>", sprintf(__('Post thumbnail (you can set thumbnail size by editing line %s in file %s)', 'wp_carousel'), $wp_carousel_thumbnail_size_line, '<code>'.__FILE__.'</code>'), "</li><li>", __('First image in the post', 'wp_carousel'), "</li></ol></p>");
		
		echo "<h5>".__('Posts\'s video for the carousel', 'wp_carousel')."</h5>";
		printf("%s %s %s %s %s %s %s", "<p>", __('WP Carousel loads the video for the carousel in the following order:', 'wp_carousel'), "<ol><li>", __('Value of custom field', 'wp_carousel'), " <em>".WP_CAROUSEL_VIDEO_URL."</em></li><li>", __('First video in the post', 'wp_carousel'), "</li></ol></p>");
		
		echo "<h5>".__('Posts\'s text for the carousel', 'wp_carousel')."</h5>";
		printf("%s %s %s %s %s %s %s", "<p>", __('WP Carousel loads the text for the carousel in the following order:', 'wp_carousel'), "<ol><li>", __('Value of custom field', 'wp_carousel'), " <em>".WP_CAROUSEL_CAROUSEL_TEXT."</em></li><li>", __('Posts\'s exceprt', 'wp_carousel'), "</li></ol></p>");
		echo '<hr class="wp_carousel_help_separator" />';
		
		echo "<h5>".__('When I add a post with a video and an image, the carousel shows the video instead of the image', 'wp_carousel')."</h5>";
		echo "<p>";
		printf(__('When a post has a video and an image, WP Carousel shows the video instead of the image. You can reverse this behaviour by replacing code in line %s in file <code>%s</code> by this: <code>%s</code>', 'wp_carousel'), $wp_carousel_videos_first_line, __FILE__, "define('WP_CAROUSEL_SHOW_VIDEOS_FIRST', false);");
		echo "</p>";
		echo '<hr class="wp_carousel_help_separator" />';
		
		echo "<h5>".__('I do not want WP Carousel to save backups', 'wp_carousel')."</h5>";
		echo "<p>";
		printf(__('To disable autosaving backups, simply replace line %s in file <code>%s</code> by this: <code>%s</code>', 'wp_carousel'), $wp_carousel_autosave_backups_line, __FILE__, "define('WP_CAROUSEL_AUTOSAVE_BACKUPS', false);");
		echo "</p>";
		echo '<hr class="wp_carousel_help_separator" />';  
		
		echo "<h5>".__('Where can I download more Themes and Extras for WP Carousel?', 'wp_carousel')."</h5>";
		printf(__('You can download more Addons for WP Carousel <a href="%s">here</a>.', 'wp_carousel'), WP_CAROUSEL_THEME_SITE_URL);
	...
	function wp_carousel($id, $mode='show', $force_theme = false)
	{
		...
		*/
		...
		if (isset($items[$id]) && isset($config[$id])):
			...
			if (!isset($config['USE_JCAROUSEL']))
			{
				$config['USE_JCAROUSEL'] = false;
			}
			else
			{
				if ($config['USE_JCAROUSEL'] == 1)
				{
			...
			if (count($items) > 0)
			{
				...
				switch ($mode)
				{
					...
					case 'show':
						...
						{
							require('themes/'.$config['THEME'].'/theme-jcarousel.php');
						}
						else
						{
	...
	*/
	...
	function wp_carousel_load_carousel_js($id)
	{
		...
		if (isset($config[$id]))
		{
			...
			if (isset($current_theme['supports']['jcarousel']) && isset($value['USE_JCAROUSEL']))
			{
			
				if ($current_theme['supports']['jcarousel'] && $value['USE_JCAROUSEL'] == 1)
				{
					$use_stepcarousel = false;
				}
				else
				{
					$use_stepcarousel = true;
				}
		...
		*/
		...
		if(is_numeric($this_carousel['ID']))
		{
			...
			if ($will['UPDATE_BOTH_WP_CAROUSEL_OPTION_AND_CONFIG'])
			{
				...
				$log[] = 'CONTENIDO ANTERIOR: "'.get_option(WP_CAROUSEL_ITEMS_TABLE).'"';
				...
				$log[] = 'CONTENIDO ANTERIOR: "'.get_option(WP_CAROUSEL_CONFIG_TABLE).'"';
	...
	function wp_carousel_get_video_embed_code($id, $mode='get')
	{
		...
		if ($video_embed_code == '')
		{
			...
			foreach ($matches as $key => $value)
			{
				...
				$final_match = str_replace("\n", '', str_replace('<p>', '', str_replace('</p>', '', apply_filters('the_content', $value))));
	...
	function wp_carousel_item_value($id, $type, $value, $items=array(), $mode='get', $debug=false)
	{
		...
		switch ($type)
		{
			...
			case '2':
				...
				break;
			case '3':
				$log[] = 'La variable $type indica que se trata de una pagina o de un articulo';
				// Cargamos en $return['name'] el nombre del elemento
				$return['name'] = get_the_title($id);
				$log[] = 'Se ha cargado el nombre del elemento en la variable $return[\'name\'], que ahora vale: "'.$return['name'].'"';
				// Cargamos en $return['image_url'] la URL de la imagen
				...
				if (function_exists('get_the_post_thumbnail'))
				{
					$log[] = 'La funcion get_the_post_image existe, se procede a cargarla como imagen del elemento';
					$img_url_temp = get_post_thumbnail_id($id);
					$attachment_array = wp_get_attachment_image_src($img_url_temp, WP_CAROUSEL_DEFAULT_THUMBNAIL_SIZE);
					$return['image_url'] = $attachment_array[0];
					$log[] = 'Se ha cargado la URL de la miniatura del elemento en la variable $return[\'image_url\'], que ahora vale: "'.$return['image_url'].'"';
				...
				if ($return['image_url'] == '')
				{
					$log[] = 'El elemento no tenia miniatura o WordPress no admite esta funcionalidad, se procede a cargar la URL de la imagen del  campo personalizado: '.WP_CAROUSEL_IMAGE_URL.'';
					$return['image_url'] = get_post_meta($id, WP_CAROUSEL_IMAGE_URL, true);
					$log[] = 'El valor del campo personalizado '.WP_CAROUSEL_IMAGE_URL.' es: "'.$return['image_url'].'"';
				}
				if ($return['image_url'] == '')
				{
					$log[] = 'El campo personalizado no tenía un valor adecuado, se toma la primera imagen del elemento para el carrusel';
					$return['image_url'] = wp_carousel_first_image($id, 'get');
					$log[] = 'La primera imagen del elemento tiene por URL: "'.$return['image_url'].'"';
				}
				$log[] = 'Ha finalizado la obtencion de la URL de la imagen del elemento, que es: "'.$return['image_url'].'"';
				// Obtenemos la URL del enlace del artículo
				$return['link_url'] = get_post_meta($id, WP_CAROUSEL_LINK_URL, true);
				$log[] = 'La URL indicada en el campo personalizado '.WP_CAROUSEL_LINK_URL.' es: "'.$return['link_url'].'"';
				if ($return['link_url'] == '')
				{
					$return['link_url'] = get_permalink($id);
					$log[] = 'La URL del elemento se ha tomado de la URL del enlace, que es: "'.$return['link_url'].'"';
				}
				// Ahora cargamos el valor del campo personalizado
				...
				break;
			case '4':
				$log[] = 'La variable $type indica que se trata de un contenido personalizado';
				// Cargamos en $return['name'] el nombre del elemento
				...
				$log[] = 'La URL del elemento es: "'.$return['link_url'].'"';
	
				$return['desc'] = $items[$id]['DESC'];
				$log[] = 'Se ha asignado como descripcion el valor: "'.$return['desc'].'"';
				break;
			case 5:
		...
		switch ($value)
		{
			...
			case 'image_url':
				...
				break;
			case 'video_url':
				$log[] = 'La variable $value tiene por valor: "image_url", así que procederemos a mostrarla';
				break;
			default:
		...
		$log[] = 'Se elimina de la variable $return[$value] cualquier aparicion del Shortcode [wp_carousel], para evitar bucles infinitos';
	...
	function wp_carousel_themes_options_area($id, $debug=false)
	{
		...
		<ul id="themes_carousel" class="jcarousel-skin-themes_carousel">
			<?php wp_carousel_list_themes($id); ?>
				...
				<li class="more_themes">
				
					<h4>
						<span class="theme-name"><?php _e('More themes', 'wp_carousel'); ?></span>
						<br />
						<span class="theme-author"><?php echo ' ('.__('by other users', 'wp_carousel').')'; ?></span>
					</h4>

					<p class="theme-description">
						<?php _e('Are you looking for more themes for WP Carousel?', 'wp_carousel'); ?>
						<br  /><br  />
						<?php _e('You\'ll find more themes created by other users at the Addons Directory.', 'wp_carousel'); ?>
						<hr class="theme-info-separator" />
						<span class="action-links">
							<a href="<?php echo WP_CAROUSEL_THEME_SITE_URL; ?>"><?php _e('Addons Directory', 'wp_carousel'); ?></a>
						</span>
					</p>
			...
			<div id="standard_options">
				<form name="standard_options" method="post" id="theme_standard_options" onsubmit="return wp_carousel_update_ajax_standard_options()">
					...
					?>
						...
						if ($current_theme['supports']['jcarousel'])
						{
							
						?>
						
							<tr valign="top">
								<th colspan="2"><h3><?php _e('jCarousel', 'wp_carousel'); ?></h3></th>
							</tr>
						
							<tr valign="top">
								<th scope="row"><label for="use_jcarousel"><?php _e('Enable jCarousel (WP Carousel will use it instead of StepCarousel)?', 'wp_carousel'); ?></label></th>
								<td><input type="checkbox" name="use_jcarousel" id="use_jcarousel" value="yes"<?php if ($config['USE_JCAROUSEL'] == "1") { echo ' checked="checked"'; } ?> /></td>
							</tr>
						...
						foreach ($current_theme['supports'] as $s_key => $s_value)
						{
							...
							<tr valign="top">
								<th scope="row"><label for="autoslide_posts"><?php echo _e('Posts moved in each autoslide (only for StepCarousel) (0 for disable autoslides)', 'wp_carousel'); ?></label></th>
								<td>
	...
	*/
	...
	function wp_carousel_list_themes($id, $debug=false)
	{
		...
		else
		{
			...
			$log[] = 'Se ha cargado la informacion sobre los themes disponibles en la matriz $themeinfo';
						...
						<div class="theme-<?php echo $value['name']; ?> info-icon">
							<div class="info-container">
								<?php
									...
									if ($value['supports']['stepcarousel'])
									{
										echo '<li>StepCarousel ';
