<?php
	...
	*/
	...
	// 0.91 = FIRST BETA VERSION -> It will be 1.00 when finished :)
	define('WP_CAROUSEL_VERSION', 0.92); // 0.6 = 0.60 < 0.6X < 0.70 == 0.7
	...
	*/
	...
	/*
		Change this value to FALSE to disable AJAX Carousel Loading
	*/
	
	define('WP_CAROUSEL_AJAX_LOADING', true);
					...
					if (isset($extra['js']))
					{
						if (is_array($extra['js']))
						{
							foreach ($extra['js'] as $js_key => $js_file)
							{
								...
								function load_plugin_".$temp_value."_js_file_".$js_key."()
								{
								...
								add_action('wp_footer', 'load_plugin_".$temp_value."_js_file_".$js_key."');
	...
	*/
	...
	{
		global $wp_carousel_thumbnail_size_line, $wp_carousel_external_integration_line;
		...
		printf("%s %s %s %s %s %s %s %s %s", "<p>", __('WP Carousel loads the image for the carousel in the following order:', 'wp_carousel'), "<ol><li>", __('Value of custom field', 'wp_carousel'), " <em>".WP_CAROUSEL_IMAGE_URL."</em></li><li>", sprintf(__('Post thumbnail (you can set thumbnail size by editing line %s)', 'wp_carousel'), $wp_carousel_thumbnail_size_line), "</li><li>", __('First image in the post', 'wp_carousel'), "</li></ol></p>");
	...
	*/
	...
	/*
		...
		@Parámetros:
								...
								$mode (show | get | array | carousel_ei |shortcode): Dependiento del valor, muestra el carrusel (show), lo devuelve (get), lo devuelve evitando modificar el bucle de WordPress (shortcode) o devuelve una matriz con su contenido (array).
					...
					{					
						...
						else
						{
							...
							if (!isset($items_adapted[$key]['VIDEO']))
							{
								...
								{
									$items_adapted[$key]['VIDEO'] = wp_carousel_get_video_embed_code($value, 'get');
								}
							}
	...
	function wp_carousel_list_themes($id, $debug=false)
	{
		...
		else
		{
			...
			foreach ($themeinfo as $key => $value)
			{
				...
				if ($counter == $count && strrpos($_SERVER['HTTP_USER_AGENT'], 'Firefox'))
				{
					$total++;
				} 
				...
				?>
				<li<?php if ($key == $config['THEME']) { echo ' class="current_theme"'; } if ($hide_this) { echo ' style="display:none"'; }?>>
					...
					<p class="theme-description">
						...
						<span class="action-links">
							...
							{
							?>
							<a href="<?php echo wp_carousel_create_internal_urls('SELF_URL:SAVE_ONLY_FIRST_URL_VARIABLE').'&action=UPDATE_THEME:'.$key; ?>#jump_themes"><?php echo __('Activate', 'wp_carousel'); ?></a>
							<?php } ?>
							 | 
							<a href="<?php echo $value['url']; ?>"><?php echo __('Theme\'s site', 'wp_carousel'); ?></a>
							 | 
							<a href="<?php echo $value['author_url']; ?>"><?php echo __('Author\'s site', 'wp_carousel'); ?></a>
						</span>
						<hr class="theme-info-separator" />
