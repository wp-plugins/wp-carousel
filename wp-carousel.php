<?php
	...
	*/
	...
	wp_register_script('wp_carousel_stepcarousel', $path[6].'js/stepcarousel.js', array('jquery')); // Stepcarousel, el script JS principal del carrusel (tomado de http://www.dynamicdrive.com/dynamicindex4/stepcarousel.htm)
	wp_register_script('wp_carousel_init_stepcarousel', $path[6].'js/init.stepcarousel.php', array('wp_carousel_stepcarousel')); // Configuraciones de los carruseles
	...
	*/
	...
	{
		/*
			Cargamos el código JS del carrusel
		*/
		if (!is_admin())
		{
			wp_enqueue_script('wp_carousel_init_stepcarousel');	// Vale, puede que no os guste que cargue siempre el código, pero no tengo ni idea de cómo detecar cuándo se ha de mostrar y cuándo no. Si vosotros sabéis cómo hacerlo, avisadme y lo corregiré.
	...
	/*
		...
		@Parámetros:
								...
								$return_array: Si es true, devuelve la matriz con todas las variables, mientras que si es false, carga el theme y muestra el carrusel. Por defecto es false.
		@Descripción: Muestra el carrusel con ID $id.
		@Añadida en la versión: 0.1		
	*/
	
	function wp_carousel($id, $return_array=false)
	{
