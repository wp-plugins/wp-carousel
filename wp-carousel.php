<?php
	...
	*/
	...
	define('WP_CAROUSEL_VERSION', 0.91); // 0.6 = 0.60 < 0.6X < 0.70 == 0.7
		...
		else
		{
			...
			function wp_carousel_print_edit_carousel_page_css()
			{
				...
				jQuery(document).ready(function($) {
					...
					/*
					$("#theme_options :checkbox").iphoneStyle({
						checkedLabel: "<?php _e('Yes', 'wp_carousel'); ?> ", 
						uncheckedLabel: "<?php _e('No', 'wp_carousel'); ?>",
						resizeContainer: false,
						resizeHandle: true
	...
	*/
	...
	function wp_carousel_custom_help_tab_filter($help)
	{
		...
		echo '<p>';
		printf(__('You can show a carousel by three ways. One way is adding <code>%s</code> in your WordPress theme. Other way is adding the WP Carousel\'s Widget into the sidebar. The last one is adding the shortcode <code>%s</code> in the post / page you want to show the carousel.', 'wp_carousel'), 'wp_carousel(CAROUSEL_ID)', '[wp_carousel]ID[/wp_carousel]');
		echo '</p>';
	...
	function wp_carousel_adapt_items($items)
	{
		...
		foreach ($items as $key => $value)
		{
			...
			elseif ($items_adapted[$key]['TYPE'] == 5)
			{
				...
				else // Ordenamos en orden cronológico inverso
				{
					foreach ($items_temp_adapted as $old_temp_key => $temp_value)
					{
						$old_temp_array_key = explode('_', $old_temp_key);
	...
	function wp_carousel_item_value($id, $type, $value, $items=array(), $mode='get', $debug=false)
	{
		...
		switch ($type)
		{
			...
			case '4':
				...
				if (isset($items[$id]['VIDEO_URL'])) 
				{
					$return['video_url'] = $items[$id]['VIDEO_URL'];
				} 
				else
				{
					$log[] = 'La matriz $items[\''.$id.'\'] no contiene el indice VIDEO_URL';
					$errors[] = 'La matriz $items[\''.$id.'\'] no contiene el indice VIDEO_URL';
					$log[] = 'Se ha añadido a la matriz de errores el error '.key($errors).': "'.current($errors).'"';
					$return['video_url'] = '';
				}
	...
	*/
	...
	function wp_carousel_backup_page()
	{
		...
		<div class="wrap">
			...
			<pre><?php //print_r($backups_per_day); ?></pre>
			...
			<table id="backup_table" cellspacing="0">
				...
				<tbody>
					<?php if (is_array($backups)): ?>
					<?php foreach ($backups_per_day as $day_of_the_backup => $backups_in_this_day): ?>
						<tr>
							...
							<?php
								...
								foreach ($backups_in_this_day as $carousel_id => $backups_in_this_carousel):
									...
									while ($loop_counter < $carousel_id)
									{
										echo '<td></td>';
										$loop_counter++;
									}
	...
	*/
	...
	class WP_Carousel_Widget extends WP_Widget
	{
		...
		function form($instance)
		{
			...
			<?php 
			/*<p><input class="widefat" id="<?php echo $this->get_field_id('id'); ?>" name="<?php echo $this->get_field_name('id'); ?>" type="text" value="<?php echo $id; ?>" /></label></p>*/
		}
