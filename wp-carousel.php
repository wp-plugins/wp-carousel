<?php
	...
	*/
	...
	?>
		...
		<?php $wp_carousel_more_content = get_option('wp_carousel_more_content'); ?>
		
		<?php if($category_carousel != 0 && $num_carousel != 0 || $wp_carousel_more_content != '') { ?>
		...
		<div id="wp_carousel" class="carousel">
			...
			<div class="belt">
				...
				<?php foreach ($wp_carousel_more_content as $key => $value) { ?>
				
					<?php if ($value['image_url'] != '' && $value['link_url'] != '' && $value['text'] != '') { ?>
				
						<div class="panel">
							<a href="<?php echo $value['link_url']; ?>"><img src="<?php echo $value['image_url']; ?>" <?php if ($same_width == 'yes') { echo 'width="'.$width.''.$unit.'"'; } ?> <?php if ($more_height == 'yes') { echo 'height="'.$more_height_h.''.$more_height_unit.'"'; } ?> /></a>
	...
	function wp_carousel_crear_pagina_opciones() {
		...
		if (isset($_POST['wp_carousel_post_cat_id'])) {
			...
			update_option('wp_carousel_image_more_height_h_unit', $_POST['wp_carousel_image_more_height_h_unit']);
		...
		if (isset($_POST['wp_carousel_more_content_1_link_url'])) {
			...
			do {
				$wp_carousel_more_content[$n]['image_url'] = $_POST['wp_carousel_more_content_'.$n.'_image_url'];
				$wp_carousel_more_content[$n]['link_url'] = $_POST['wp_carousel_more_content_'.$n.'_link_url'];
				$wp_carousel_more_content[$n]['text'] = $_POST['wp_carousel_more_content_'.$n.'_text'];
			...
			update_option('wp_carousel_more_content', $wp_carousel_more_content);
    ...
    <td><input name="wp_carousel_panel_width" id="wp_carousel_panel_width" value="<?php echo get_option('wp_carousel_panel_width'); ?>" /> <select name="wp_carousel_panel_width_unit"><option value="px" id="px" <?php if(get_option('wp_carousel_panel_width_unit') == 'px') { echo 'selected="selected" '; } ?>>Px</option><option value="percent" id="percent" <?php if(get_option('wp_carousel_panel_width_unit') == 'percent') { echo 'selected="selected" '; } ?>>%</option></select></td>
	</tr>
    ...
    <td><select name="wp_carousel_image_more_height"><option value="yes" id="yes" <?php if(get_option('wp_carousel_image_more_height') == 'yes') { echo 'selected="selected" '; } ?>>Yes</option><option value="no" id="no" <?php if(get_option('wp_carousel_image_more_height') == 'no') { echo 'selected="selected" '; } ?>>No</option></select>
		...
		<td><input name="wp_carousel_image_more_height_h" id="wp_carousel_image_more_height_h" value="<?php echo get_option('wp_carousel_image_more_height_h'); ?>" /> <select name="wp_carousel_image_more_height_h_unit"><option value="px" id="px" <?php if(get_option('wp_carousel_image_more_height_h_unit') == 'px') { echo 'selected="selected" '; } ?>>Px</option><option value="percent" id="percent" <?php if(get_option('wp_carousel_image_more_height_h_unit') == 'percent') { echo 'selected="selected" '; } ?>>%</option></select></td>
		</tr>
	...
	<?php 
		...
		$wp_carousel_more_content = get_option('wp_carousel_more_content');
    ...
    <tr valign="top" class="wp_carousel_more_content" title="<?php echo $n; ?>">
		...
		<th scope="row">
			...
			<h3>Costumized content #<?php echo $n; ?></h3>
		...
		<tr valign="top">
			...
			<td><input name="wp_carousel_more_content_<?php echo $n; ?>_image_url" id="wp_carousel_more_content_<?php echo $n; ?>_image_url" value="<?php echo $wp_carousel_more_content[$n]['image_url']; ?>" /></td>
		...
		<tr valign="top">
			...
			<td><input name="wp_carousel_more_content_<?php echo $n; ?>_link_url" id="wp_carousel_more_content_<?php echo $n; ?>_link_url" value="<?php echo $wp_carousel_more_content[$n]['link_url']; ?>" /></td>
		...
		<tr valign="top">
			...
			<td><textarea name="wp_carousel_more_content_<?php echo $n; ?>_text" id="wp_carousel_more_content_<?php echo $n; ?>_text"><?php echo $wp_carousel_more_content[$n]['text']; ?></textarea></td>
	...
	<?php
			$n++;
		} while ($wp_carousel_more_content[$n]['image_url'] != '');
	?>
	
	<tr valign="top">
	<th scope="row" colspan="2">
	<a href="#" id="wp_carousel_more_personalized_content">Add more costumized content</a> (erase the fields to delete custom content)
	</th>
	</tr>
