...
?>
					...
					if (WP_CAROUSEL_SHOW_VIDEOS_FIRST)
						{
							if ($there_is_video)
							{
								echo apply_filters('the_content', '<div class="video_margin">[embed width="'.str_replace('px', '', $temp_panel_width).'"]'.$item['VIDEO'].'[/embed]</div>');
							}
							else
							{
								?>
	...
	<div id="carousel_<?php echo $c_id; ?>-paginate" class="jcarousel wp_carousel_premium-slider_pagination" style="width:<?php echo $temp_panel_width; ?>;">
		<?php $t_id = 0; ?>
		<?php foreach ($items as $i_id => $item): ?>
			<?php $t_id++; ?>
